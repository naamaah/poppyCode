...
def wf_joints(ex_name, list_joints):
    '''
    Writing joints data for an exercise in Excel file - after each exercise finished in camera class.
    :param ex_name:
    :param list_joints:
    :return:
    '''
...
# write to execl file exercises names and the successful repetition number one time in the end of the workout (Poopy class)
def wf_exercise():
...
def close_workbook():
    s.excel_workbook.close() #only after this create the excel file.
...
if __name__ == "__main__":
    ...
    # wf_joints("hands horizontally",join)
    # close_workbook()
